	...
	$this->widget('SimplaPager', array('pages'=>$pages)); ?>
		...
		<table class="product-excel-style" style="width:760px">
			<tr>
				...
				<th><? echo Yii::t('supplier_message', 'supplier_cd'); ?></th>
				...
				<th><? echo Yii::t('supplier_message', 'remark'); ?></th>
		...
		<? foreach($items as $supplier) { ?>
			<tr>
				...
				<td><input type="button" value="<? echo Yii::t('common_message', 'delete'); ?>" onclick="javascript:goDelete('<?=$supplier->id ?>')" /></td>
				<td><?=$supplier->name ?></td>
